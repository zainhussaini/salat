...
def eot_decl(time: dt.datetime) -> "tuple[dt.timedelta, float]":
    ...
    epoch = dt.datetime(2000, 1, 1, 12, tzinfo=gmt)
...
def calc_dhuhr(date: dt.date, timezone: dt.timezone, longitude: float) -> dt.datetime:
    """Calculates time of Dhuhr on date at longitude, in the timezone given.

    Args:
        date (date): The date to calculate Dhuhr time for
        timezone (timezone): The timezone of the given date and the output
    ...
    dhuhr_offset = timezone.utcoffset(None) - dt.timedelta(hours=longitude / 15)

    dhuhr = noon + dhuhr_offset - eot
    return dhuhr
...
def time_alt_first(
    ...
    # TODO: make altitude use above the horizon instead of below
    """Calculates the first time on the time before dhuhr when the altitude of
    the sun is as given.
    ...
    Returns:
        datetime: The time immediately before dhuhr when altitude is as given
    """
    ...
    # start guesses at midnight and dhuhr
    guess1 = dt.datetime(date.year, date.month, date.day, 0, tzinfo=timezone)
    guess2 = dhuhr
...
def time_sf_first(
    ...
    """Calculates the first time in the day
...
def linear_interpolation(
    ...
    while not math.isclose((guess1 - guess2).total_seconds(), 0):
        # # make sure that diff1 and diff2 have different signs
        # assert (diff1 < timedelta()) != (diff2 < timedelta())
...
def calc_altitude(shadow_factor: float, declination: float, latitude: float) -> float:
    """Calculates altitude when shadow of object is show_factor times longer
    than the object.
